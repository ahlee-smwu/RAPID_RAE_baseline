#!/usr/bin/env bash
# Launch experiment 0 (baseline) or 1 (rapid) on all 5 GPUs, XL config.
#   bash learnable_eps/run_xl.sh baseline|rapid [extra train.py args...]
# GPUs: $GPUS (default all five), $NPROC (default 5) must equal the number of GPUs listed.
set -euo pipefail
cd "$(dirname "$0")/.."
arm=${1:?baseline|rapid}; shift || true
export CUDA_VISIBLE_DEVICES=${GPUS:-0,1,2,3,4}
export LAT=${LAT:-/home/ahlee/latents_fp16}   # fp16 copy (convert_latents_fp16.py); fp32 original: /mnt/aisha/ahlee-rae
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
mkdir -p logs_rapid
exec torchrun --standalone --nnodes=1 --nproc_per_node=${NPROC:-5} \
