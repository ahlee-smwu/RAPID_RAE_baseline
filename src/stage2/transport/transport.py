...
class Transport:
    ...
    # ------------------------------------------------------------------
    # RAPID adaptive prior training path.
    # ------------------------------------------------------------------

    def _ot_pair_within_class(self, x0_gmm, x1, y):
        """Hungarian matching between x0_gmm and x1, restricted to each class.

        Returns the permutation index so the CALLER can reorder x1 and y
        together.

        Why the permutation is returned rather than applied to x1 alone:
        RAPID's training_losses_learnable_eps2 does ``x1 = x1[col_idx]`` but
        leaves ``model_kwargs['y']`` untouched. On a single-class dataset
        (LSUN) that is harmless, but on ImageNet it trains image i's latent
        under a DIFFERENT class label -- a silent label-shuffling bug. Keeping
        the permutation explicit here makes it impossible to reorder one
        without the other.
        """
        from scipy.optimize import linear_sum_assignment

        B = x1.shape[0]
        perm = th.arange(B, device=x1.device)
        if y is None:
            return perm

        x0_flat = x0_gmm.reshape(B, -1).float()
        x1_flat = x1.reshape(B, -1).float()

        for label in th.unique(y):
            idx = th.nonzero(y == label, as_tuple=False).squeeze(1)
            if idx.numel() < 2:
                continue
            cost = th.cdist(x0_flat[idx], x1_flat[idx])
            _, col = linear_sum_assignment(cost.detach().cpu().numpy())
            perm[idx] = idx[th.from_numpy(col).to(x1.device)]
    ...
    def training_losses_gmm(
        ...
        decay_alpha=1.0,
        schedule="exp",
        ot_within_class=False,
    ):
        """Velocity loss on the GMM-blended adaptive prior path.

        Args:
        - x1: datapoint (the RAE latent the model is trained on)
        - x0_gmm: per-sample GMM prior draw, same shape as x1
        - q0 / decay_alpha / schedule: blending schedule, see ICPlan.gmm_weight_t
        - ot_within_class: Hungarian-match x0_gmm to x1 inside each class group

        The standard-Gaussian draw produced by ``self.sample`` is REUSED as the
        blend's eps rather than drawing a second one -- at 196608-D per sample
        a redundant randn is a real cost.
        """
        if self.model_type != ModelType.VELOCITY:
            raise NotImplementedError(
        ...
        if schedule not in ("exp", "const"):
            # Do not fall back silently: a typo'd schedule would otherwise run
            # a different experiment than the config claims.
            raise NotImplementedError(
        ...
        if ot_within_class:
            ...
            perm = self._ot_pair_within_class(x0_gmm, x1, y)
            # Reorder x1 AND the conditioning label together (see docstring).
            x1 = x1[perm]
            if y is not None:
                model_kwargs = dict(model_kwargs)
                model_kwargs['y'] = y[perm]

        if schedule == "const":
            t, xt, ut = self.path_sampler.plan_gmm_const(
                t, x0_gmm, x1, q_const=q0, eps=x0
            )
        else:
            t, xt, ut = self.path_sampler.plan_gmm_adaptive(
                t, x0_gmm, x1, q0=q0, decay_alpha=decay_alpha, eps=x0
            )
